...
def centroid_differences_np(prediction):
    '''Compute centroid differences for connected components.'''
    num_labels = prediction.shape[-1]
    dist = 0
    for struc_id in range(1, num_labels):

        blobs = measure.label(prediction[...,struc_id], connectivity=1)
        props = measure.regionprops(blobs)

        if len(props) <= 1:
            continue

        centroids = np.array([el.centroid for el in props])
        dist_vecs = np.linalg.norm(centroids[:-1] - centroids[1:], axis=1)
        dist += np.sum(dist_vecs)

    return np.float32(dist)


def connected_component_loss(logits, labels):
    """
    Custom loss function that computes the distance between
    ...
    mask = tf.one_hot(tf.argmax(softmax, axis=-1), depth=tf.shape(softmax)[-1])

    result = tf.py_func(centroid_differences_np, [mask], tf.float32)
    return result
